...
const AIAnalyzer: React.FC<AIAnalyzerProps> = ({ cipherState, onAnalysisComplete }) => {
  ...
  const calculateFrequencyAnalysis = (text: string) => {
    ...
    return Object.entries(freq)
      .map(([character, count]) => {
        const frequency = Math.round((count / text.length) * 100 * 100) / 100
        return {
          character,
          frequency,
          expected: 100 / Object.keys(freq).length, // Expected frequency for uniform distribution
          deviation: Math.abs(frequency - (100 / Object.keys(freq).length))
        }
      })
  ...
  const generateRecommendations = (strength: any, predictions: any[]): string[] => {
    const score = strength.overall || 0
    ...
    if (predictions.some(p => p.impact === 'high' || p.impact === 'critical')) {
                    ...
                    results.aiPredictions.map((prediction, index) => (
                      <div key={index} className="flex items-start space-x-3 p-3 bg-muted/30 rounded-lg">
                        <AlertTriangle className={`w-5 h-5 mt-0.5 ${
                          prediction.impact === 'critical' || prediction.impact === 'high' ? 'text-red-500' : 
                          prediction.impact === 'medium' ? 'text-yellow-500' : 'text-green-500'
                        }`} />
                        ...
                        <Badge variant={prediction.impact === 'critical' || prediction.impact === 'high' ? 'destructive' : 'secondary'}>
                          {prediction.impact}
                        </Badge>
                      </div>
                    ))
