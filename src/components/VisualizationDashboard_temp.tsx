...
const VisualizationDashboard: React.FC<VisualizationDashboardProps> = ({ cipherState }) => {
  ...
  const renderCustomizedLabel = ({
    cx, cy, midAngle, innerRadius, outerRadius, bit
  }: any) => {
  ...
  return (
    ...
    >
      <div className="flex items-center space-x-3 mb-8">
        ...
        <h2 className="text-2xl font-semibold text-foreground">Cryptographic Analysis Dashboard</h2>
      </div>
              ...
              <div className="lg:col-span-1">
                <div className="space-y-2">
                  ...
                  {bitDistribution.map((entry, index) => (
                    <div key={entry.bit} className="flex items-center space-x-2 text-sm">
                      <div 
                        className="w-3 h-3 rounded-full" 
                        style={{ backgroundColor: COLORS[index % COLORS.length] }}
                      />
                      <span className="text-foreground">{entry.bit}</span>
